...
document.querySelector("#add").addEventListener("submit",function(e){
	...
	lista.splice(pos.value,0,new Articulo(cod.value,nom.value,pre.value,can.value,des.value));
	...
	imprimirActividad("Agregar: se agregó un Articulo en la posicion "+pos.value);
	mostrarLista(lista);
	pos.setAttribute("max",lista.length);
});
...
document.querySelector("#delete").addEventListener("submit",function(e){
	...
	if (indice){
		...
		document.querySelector("#posicion").setAttribute("max",lista.length);
	}
	else
